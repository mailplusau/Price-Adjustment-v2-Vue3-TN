...
import { checkSubset, formatPrice, pricingRuleOperatorOptions, rules } from "@/utils/utils.mjs";
...
<template>
    <v-container fluid v-if="userStore.isAdmin">
        <v-form class="v-row justify-center" ref="mainForm" v-model="formValid">
            <v-col cols="12" class="text-h5 text-center text-primary">Pricing Rule Record</v-col>
                    ...
                    <v-list-item v-for="(pricingRule, index) in servicePricingRules" :key="'rule' + index">
                        ...
                        <template v-slot:append>
                            <ButtonWithConfirmationPopup tooltip="Delete rule" message="Delete this pricing rule?"
                                                         @confirmed="priceAdjustmentRule.removeRule(index)">
                                <template v-slot:activator="{ activatorProps }">
